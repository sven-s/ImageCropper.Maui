...
public class PlatformImageCropper : IImageCropperWrapper
{
    public void ShowFromFile(ImageCropper imageCropper, string imageFile)
    {
        ...
        cropViewController.OnDidCropToRect = (outImage, cropRect, angle) => { Finalize(imageCropper, outImage); };
        ...
        UIApplication.SharedApplication.KeyWindow.RootViewController.PresentViewController(cropViewController, true,
            null);
    ...
    private static async void Finalize(ImageCropper imageCropper, UIImage image)
    {
        ...
        {
            imageCropper.Logger?.LogError("Failed to save cropped image to {Path}: {Error}", jpgFilename,
